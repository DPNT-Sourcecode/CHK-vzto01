...
public class CheckoutSolution {
    ...
    static int groupDiscount(HashMap<Character, Integer> items) {
        int itemTotal = items.getOrDefault('S', 0) 
        + items.getOrDefault('T',0) 
        + items.getOrDefault('X',0) 
        + items.getOrDefault('Y',0) 
        + items.getOrDefault('Z',0);
        if (itemTotal >= 3) {
            int discountNo = (Math.floorDiv(itemTotal, 3));
            for (int i = (discountNo * 3); i > 0; i--) {
                if (items.getOrDefault('Z',0) > 0) {
                    items.put('Z', (Math.max(items.get('Z') -1, 0)));
                } else if (items.getOrDefault('S',0) > 0) {
                    items.put('S', (Math.max(items.get('S') -1, 0)));
                } else if (items.getOrDefault('T',0) > 0) {
                    items.put('T', (Math.max(items.get('T') -1, 0)));
                } else if (items.getOrDefault('Y',0) > 0) {
                    items.put('Y', (Math.max(items.get('Y') -1, 0)));
                } else if (items.getOrDefault('X',0) > 0) {
                    items.put('X', (Math.max(items.get('X') -1, 0)));
                }
            }
